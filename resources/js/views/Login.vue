<template>
    <div>
         <button
            class="shadow-lg "
            style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%);    background: white; padding: 20px; border: none;"
            v-if="flagCallLogin"
            @click.prevent="loginGoogle">
            Đăng nhập với goole .... <span class="loader-login"></span>
        </button>
         <button
            class="shadow-lg "
            style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%);    background: white; padding: 20px; border: none;"
            v-else>
            Đang đăng nhập , vui lòng chờ  .... <span class="loader-login"></span>
        </button>
    </div>
</template>
  ...
  methods: {
    loginGoogle() {
      ...
      this.$auth
        ...
        .catch((err) => {
           this.flagCallLogin = true;
        })
    },
    async socialLogin(provider, res) {
      return await axios.post('https://api.laravel.org/api/login', {...res , driver : provider })
        .then((res) => {
            if(res.data.status == false) return alert("Login failed");
